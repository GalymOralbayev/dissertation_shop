...
namespace Shop.DataAccess
{
    public class OrderDbRepository : IOrderRepository
    {
        ...
        public IEnumerable<Order> GetOrdersInTwoDay(string userId)
        {
            var orders = shopContext.Orders.AsNoTracking().Include(o => o.OrderItems).ThenInclude(p => p.Product)
                .AsEnumerable()
                .Where(x => x.UserId == userId && (x.DateTime - DateTime.Now).Days <= 2)
        ...
        public void AddProduct(int orderId, Product product, int amount)
        {
            var order = shopContext.Orders.FirstOrDefault(x => x.Id == orderId);
            order.OrderItems.Add(new OrderItem { Order = order, Product = product, Amount = amount });
        ...
        public Order TryGetByOrderId(int orderId)
        {
            var orders = GetAll();
            foreach (var order in orders)
            {
